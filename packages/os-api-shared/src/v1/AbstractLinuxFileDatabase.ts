...
import type {BackupPermission, IFileBackupProps} from '../interfaces';
...
import type {BaseEntry, DistinctKey} from '../types/v1/baseEntry';

const defaultMapLogLevels: {
	add: LogLevelValue;
	assert_online: LogLevelValue;
	create_backup: LogLevelValue;
	delete: LogLevelValue;
	entries: LogLevelValue;
	list: LogLevelValue;
	load_output: LogLevelValue;
	no_key_error: LogLevelValue;
	replace: LogLevelValue;
	replace_key_value: LogLevelValue;
	restore_backup: LogLevelValue;
	store_output: LogLevelValue;
} = {
	add: LogLevel.Debug,
	...
	implements ICommonApiV1<Entry, DistinctKey<Entry, EntryKey>>, ApiServiceV1
{
	...
	public constructor(props?: Partial<Props>) {
		this.props = Object.assign({}, defaultBackupProps, props) as Props;
		const logLevels = {...defaultMapLogLevels, ...props?.logLevels} as InferLogLevels<Props>;
		this.logger = new MapLogger(this.props.logger, logLevels);
	}
	...
	public setLogMapping(logLevels: Partial<InferLogLevels<Props>>): void {
		this.logger.setLogMapping(logLevels as any);
	}

	public allLogMapSet(level: LogLevelValue): void {
		this.logger.allLogMapSet(level);
	}

	public allLogMapReset(): void {
