...
export const nssEntryProviderActionSchema: z.ZodType<{
	status: (typeof nssStatuses)[number];
	action: (typeof nssActions)[number];
}> = z.object({
...
export const nssEntryProviderSchema: z.ZodType<{
	provider: string;
	action?: NssEntryProviderAction;
}> = z.object({
	provider: z.string(),
...
export const nssEntrySchema: z.ZodType<{
	database: (typeof nssDatabases)[number];
	providers: NssEntryProvider[];
}> = z.object({
